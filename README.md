...
> ⚠️ **dmg 안에서 바로 실행하지 마세요.** macOS 가 앱을 읽기 전용 임시 경로로 복사해
> 격리(App Translocation)하며, 그 상태에서는 자동 업데이트가 영구히 동작하지 않습니다.
> 반드시 Applications 로 옮긴 뒤 거기서 실행하세요.

### 처음 열 때 "악성 코드가 없음을 확인할 수 없습니다" 라고 나오면

> Apple은 "Toki.app"에 사용자의 Mac에 손상을 입히거나 사용자의 개인정보에 침입할 수
> 있는 악성 코드가 없음을 확인할 수 없습니다.

Apple 공증(연 $99 개발자 프로그램)을 받지 않은 무료 앱이라 그렇습니다. 악성 코드가
발견됐다는 뜻이 아니라 **확인할 수단이 없다**는 뜻입니다. 서명 자체는 유효하며
`codesign --verify --strict /Applications/Toki.app` 으로 직접 확인할 수 있습니다.

한 번만 허용하면 됩니다. 터미널 쪽이 확실합니다:

```bash
xattr -dr com.apple.quarantine /Applications/Toki.app
```

또는 앱을 한 번 실행해 차단당한 뒤 **시스템 설정 → 개인정보 보호 및 보안** 을 열고,
맨 아래 Toki 항목의 **"확인 없이 열기"** 를 누르세요.

> 예전에 통하던 **우클릭 → 열기** 는 macOS 15 부터 막혔습니다. Toki 는 macOS 26 이상이
> 필요하므로 그 방법은 쓸 수 없습니다.
...
| "악성 코드가 없음을 확인할 수 없습니다" | 공증이 없어서입니다. 위 [처음 열 때](#처음-열-때-악성-코드가-없음을-확인할-수-없습니다-라고-나오면) 참고 |
| 자동 업데이트가 계속 실패 | dmg 안에서 실행 중일 수 있습니다. Applications 로 옮긴 뒤 거기서 실행하세요 |
