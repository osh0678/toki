...
| G9 | 이벤트 감시는 좁게, 키보드 전역 감시는 없음 | 아래 § 이벤트 모니터 참조. `CGEventTap`·`IOHIDManager`·전역 키보드 모니터는 검사로 차단 |
| G10 | 클립보드는 쓰기만, URL 은 하드코딩된 2개만 | `NSPasteboard`·`NSWorkspace.open` 은 `InstallHelpCard.swift` 로 한정. 클립보드 **읽기** API 는 검사로 차단 |

### 이벤트 모니터

패널을 "바깥을 클릭하면 닫히게" 만들려면 앱 밖의 입력을 알아야 합니다. 필요한 최소만
씁니다. 둘 다 **패널이 떠 있는 동안에만** 등록되고 닫으면 해제되며, 접근성·입력
모니터링 권한을 요구하지 않습니다.

| 모니터 | 보는 것 | 쓰는 것 |
|---|---|---|
| 전역 마우스 다운 | *다른* 앱으로 전달된 클릭 | **발생 사실만.** 이벤트 값은 버리고 좌표도 읽지 않습니다 — 전역 모니터에 도달한 것 자체가 "Toki 바깥"을 뜻하기 때문입니다 |
| 로컬 키다운 | **Toki 로 전달된** 키 이벤트만 | Esc 인지 여부. Esc 면 소비하고, 나머지는 그대로 통과시킵니다 |

전역 **키보드** 모니터는 머신 전체의 모든 키 입력을 관찰하게 되므로 사용하지 않으며,
`security-check.sh` 가 이를 차단합니다.
...
`./security-check.sh` 가 매 빌드 후 위 보증을 기계적으로 검증합니다. 소스 검사 9건 +
...
── 소스 검사 ──────────────────────────────────────
  ...
  ✓ URL 열기·클립보드는 InstallHelpCard.swift 에서만
  ✓ 클립보드 읽기 없음
  ✓ 전역 키보드 감시 없음
...
| **바깥 클릭 감지에 전역 마우스 모니터 사용** | 클릭의 *발생 사실*만 쓰고 값·좌표는 읽지 않으며, 패널이 떠 있는 동안에만 등록됩니다. 키보드는 로컬(Toki 로 전달된 것)만 봅니다. 접근성·입력 모니터링 권한은 필요하지 않습니다. | § 이벤트 모니터 로 범위 고정 + 검사로 전역 키보드 차단 |
